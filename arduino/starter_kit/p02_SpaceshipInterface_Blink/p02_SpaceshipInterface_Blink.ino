...
// Create a global variable to control wether the op 
// LEDs are in a blink state of 'on' or 'off'.
boolean blink_off = true;

// The delay for blink time in ms.
// For values around 1000ms you need to hold the switch for it work.
// For values around 100ms it works as before. 
...
void loop() {
  ...
  if (switchstate == LOW) {
    ...
    if (blink_off) {
      ...
      // NB: This will probably stop the switch working for large delays unless it is held down. 
      // How to multithread or get a timing from this thing?
      delay(BLINK_TIME);
      blink_off = false;
    }
    else {
