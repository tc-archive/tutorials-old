...
% dbg:tracer().
%
% Trace all function calls for the gen_server:
% dbg:p(all,c).
% dbg:tpl(resource_discovery, x).
%
...
%
% - registered() 
% Returns a list of names which have been registered usingregister/2.
%
% - whereis(Name) 
% Returns the pid registered under Name, orundefinedif the name is not registered.
%
...
handle_call({fetch_resources, Type}, _From, State) ->
  io:format("Fetching resources: ~p~n", [Type]),

  % {reply, dict:find(Type, State#state.found_resource_tuples), State}.
  
  Local = case dict:find(Type, State#state.local_resource_tuples) of
    % e.g: {ok,["Honda","Mazda"]}
    {ok, LocalRes} -> LocalRes;
    error -> []
  end,
  io:format("Local resources: ~p~n", [Local]),

  Found = case dict:find(Type, State#state.found_resource_tuples) of
    % e.g: {ok,["Airbus","Boeing"]}
    {ok, FoundRes} -> FoundRes;
    error -> []
  end,
