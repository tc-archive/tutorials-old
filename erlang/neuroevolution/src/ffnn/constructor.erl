...
% held in the NIds variable. In create_NeuroLayers/7, with every iteration we generate the
% Output_NIds, which are the Ids of the neurons in the next layer. The last layer is a special case
% which occurs when LayerIndex == Tot_Layers. Having the Input_IdPs, and the Output_NIds, we are able
% to construct a neuron record for every Id in NIds using the function create_layer/4. The Ids of the
% constructed Output_NIds will become the NIds variable of the next iteration, and the Ids of the
% neurons in the current layer will be extended and become Next_InputIdPs. Wethen drop into the next
% iteration with the newly prepared Next_InputIdPs and Output_NIds. Finally, when we reach the last
% layer, the Output_Ids is the list containing a single Id of the Actuator element. We use the same
% function, create_NeuroLayer/4, to construct the last layer and re- turn the result.
...
% neurons in the next layer, the Input_IdPs and Output_Ids are the same for every neuron be- longing
% to the same layer.
